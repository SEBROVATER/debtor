...
mod tests {
    ...
    struct SupervisorFake(bool);

    impl SupervisorReadiness for SupervisorFake {
        fn is_healthy(&self) -> bool {
            self.0
        }
    }
    ...
    async fn readiness_delegates_to_the_database_port() {
        ...
        let unhealthy = ReadinessService::with_supervisor(
            Arc::new(ReadinessFake(Ok(()))),
            Arc::new(SupervisorFake(false)),
        );
        assert!(matches!(
