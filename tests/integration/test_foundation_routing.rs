...
fn foundation_routes_include_health_login_and_dashboard() {
    ...
    let health = routes
        .iter()
        .find(|r| r.method == RouteMethod::Get && r.path == "/health");
    assert!(health.is_some(), "expected /health route");
    assert!(!health.unwrap().requires_auth, "health should be public");

    let login_get = routes
        .iter()
        .find(|r| r.method == RouteMethod::Get && r.path == "/login");
    ...
    let dashboard = routes
        .iter()
        .find(|r| r.method == RouteMethod::Get && r.path == "/dashboard");
    assert!(dashboard.is_some(), "expected /dashboard route");
    assert!(
        dashboard.unwrap().requires_auth,
