...
8. Given Current is selected through the safe native form, when the native response renders, Current remains in the URL and the result heading receives focus. Enhanced replacement keeps the selected radio mounted and uses one scoped polite atomic final-status announcement; individual amounts are never live regions.
9. Given an enhanced Current calculation is pending, HTMX's request class hides prior financial content and shows one scoped Updating placeholder. The completed server-rendered replacement restores complete results or the scoped no-partial failure. This transition does not dynamically change `aria-busy`, retain client-side financial data, use custom application JavaScript, inline script attributes, or a custom HTMX extension.
...
11. Given Current is unavailable, native or enhanced output contains no partial Balance or Transfer. Enhanced failures replace only the result region and retain mode-control focus; native failures follow result-heading focus rules. Revisit/reselection recalculates safely, with no manual Retry control or persisted preference.
...
- Do not add a database preference, migration, `.sqlx` change, manual refresh endpoint/control, custom JavaScript, new route, new provider/cache, or a separate Group/Participant financial read.
- Do not change stored Spending Source Currency, allocations, dates, names, archive state, or Group Currency.
- The enhanced pending projection is CSS-only: HTMX's built-in request class displays an Updating placeholder. Do not reintroduce `hx-on`, other inline event attributes, custom HTMX extensions, CSP relaxation, dynamic `aria-busy`, or client-side financial retention.
