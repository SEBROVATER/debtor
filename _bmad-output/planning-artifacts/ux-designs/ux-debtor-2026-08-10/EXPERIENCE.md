...
Debtor is one responsive, authenticated web experience for one Administrator. Participants are Group-owned accounting identities, never application users. Semantic server-rendered HTML and native links/forms are the complete interaction baseline. Pinned self-hosted HTMX core and its pinned official `response-targets` extension are the only currently approved browser-side JavaScript infrastructure and may progressively enhance section navigation and allocation previews; neither is required to complete a task. Other official extensions require explicit design and security approval before addition. Manually authored application JavaScript, inline scripts and event handlers, custom HTMX extensions, application-owned HTMX event handlers, client-side financial state, and features requiring imperative post-swap behavior are forbidden. Operation is online only.
...
| `UX-FOCUS-01` | Stable server-owned focus IDs, forward-focus matrix, retained enhanced focus for Debts success and expected errors, and allow-listed return targets | Interaction Focus Matrix |
| `UX-STATUS-01` | Stable scoped status nodes, polite atomic announcements, approved declarative `aria-busy`, transition ownership, and alert exceptions | Financial Presentation; Requests and Calculation Modes; Accessibility Floor |
...
| Component | Use | Behavioral rules |
|---|---|---|
| **Conversion notice** | Summary and Debts | One stable node per derived region uses `role="status"`, `aria-live="polite"`, and `aria-atomic="true"`. Summary toggles its result-container `aria-busy`. Enhanced Debts mode recalculation uses HTMX's request class for its visible Updating placeholder and does not dynamically toggle `aria-busy`, because application JavaScript, inline scripts/event handlers, application-owned HTMX event handlers, client-side financial state, imperative post-swap behavior, and custom extensions are forbidden. The final server-rendered result or failure announces one outcome. The result heading/container references the notice with `aria-describedby`; individual amounts are not live regions. |
...
| Component | Use | Behavioral rules |
|---|---|---|
| **Debt mode control** | Debts | Historical is the native checked default. Selecting a mode submits a safe native form; HTMX may swap the result. The mode stays in the URL and Current is not persisted. Enhanced success and expected enhanced errors retain focus on the activated radio outside the replaced result region; the server-rendered scoped status announces the complete result or no-partial failure. Native full-page responses may autofocus their result or error heading. |
| **Request status** | Enhanced and native request outcomes | One stable scoped node uses `role="status"`, `aria-live="polite"`, and `aria-atomic="true"`; its owning region toggles `aria-busy` where the approved declarative mechanism supports it. Enhanced Debts mode recalculation is the exception: its HTMX request class shows the scoped Updating placeholder without dynamic `aria-busy`, then the completed replacement announces one outcome. The pinned official `response-targets` extension declaratively routes expected `4xx`/`5xx` fragments into it. Routine pending and expected failures retain invoker focus and announce once; urgent session loss uses a server-rendered alert/heading instead. Native links/forms present the same outcomes when enhancement is absent. Application-owned HTMX event handlers, imperative post-swap behavior, custom response extensions, and unapproved official extensions are not allowed. |
...
| Updating | Keep Source Currency Group and per-Payer totals available. Keep prior converted values only when Group, current UTC month, Group Currency, conversion mode, relevant ledger revision, and corrected Source Currencies match the pending request; label them “Updating.” Otherwise show an Updating placeholder. | Use HTMX's request class to hide prior financial content and show one scoped Updating placeholder. Do not retain client-side financial values or dynamically change `aria-busy`; the final server-rendered replacement provides the complete result or no-partial failure. Enhanced success and expected enhanced errors retain the activated rate-mode radio while the scoped status announces the outcome. | Section request proceeds automatically; no manual Retry. |
...
| Debt mode | Enhanced success retains activated radio; native forward response may autofocus result heading | Enhanced expected error retains activated radio; scoped status describes failure. Native full-page error may autofocus its heading. | Encoded mode only; browser restoration when available. |
...
- Derived regions use one stable polite atomic **Conversion notice** and announce exactly one Updating, stale, provisional, Ready, or Unavailable transition rather than every amount. They toggle `aria-busy` where the approved declarative mechanism supports it; enhanced Debts mode recalculation instead uses its CSS-only Updating placeholder and final server-rendered announcement while enhanced success and expected enhanced errors retain the activated rate-mode radio. Respect reduced-motion preferences.
- General enhanced outcomes use the stable polite atomic **Request status** and owning-region `aria-busy` where supported; the official `response-targets` extension routes expected `4xx`/`5xx` fragments there. Enhanced Debts mode recalculation is the CSS-only exception defined in [Requests and Calculation Modes](#requests-and-calculation-modes). Validation summaries and urgent session loss use alert/heading treatment instead.
- Full-page forward responses use stable IDs and one server-rendered `autofocus` target. Enhanced responses follow their Interaction Focus Matrix row: routes requiring retained focus render no competing autofocus target. Back/Forward accessibility relies only on encoded state and browser-native restoration, never promised prior focus.
