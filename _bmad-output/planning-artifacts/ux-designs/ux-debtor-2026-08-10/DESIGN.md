...
| **Request status** | Stable scoped region with explicit pending, failure, and committed-state text. Routine updates use `{colors.text-muted}`; failures use `{colors.warning-text}` and `{colors.warning}` rules; committed lifecycle status uses `{colors.success}`. It never overlays controls or relies on animation. |
