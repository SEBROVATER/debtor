...
- Transport adapters decode field structure and retain raw submitted text only. Transport-neutral application inputs parse amount, currency/category code, and `%Y-%m-%d` date text; validate Participant selections, ownership, lifecycle, and payer/share modes; construct allocations; and apply all financial invariants. Web parsing must not construct financial allocations.
...
- Payer and Share allocations are nonempty and Participant-unique, zero allocations are invalid, and each side equals the Spending Total exactly in source-currency minor units. Equal-split residual minor units are assigned in ascending Participant-ID order.
...
- Render semantic HTML with Askama and vanilla CSS. Custom JavaScript is forbidden; HTMX may enhance interactions but core behavior remains server-rendered.
- One shared strict form/CSRF extractor rejects malformed, missing, duplicate, and unknown fields before route-specific parsing or use-case dispatch. CSRF rejection likewise occurs before dispatch.
...
- Login and authenticated HTML responses send `Cache-Control: no-store`, `X-Content-Type-Options: nosniff`, `Referrer-Policy: no-referrer`, and `Content-Security-Policy: default-src 'none'; script-src 'none'; style-src 'self' 'unsafe-inline'; form-action 'self'; base-uri 'none'; frame-ancestors 'none'`.
- Probe and static routes must neither create nor load sessions.
