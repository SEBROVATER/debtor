...
- Exactly one active Group-owned Participant pays the Spending Total. Share allocations are nonempty and Participant-unique, zero Shares are invalid, and Shares equal the Total exactly in Source Currency minor units. Proportional mode converts positive decimal weights through largest fractional remainder with Participant ID as tie-breaker; Exact mode begins with equal minor-unit Shares and requires a zero remaining/excess difference before acceptance.
...
- Participant archival requires a complete all-time Historical-mode calculation with an exact zero Group Currency Balance. Missing fresh or eligible stale rates block the mutation with retryable feedback. The eligibility decision and archive write must be race-safe against concurrent Spending mutations without holding a database transaction during provider calls. Restore does not require Balance eligibility.
...
- Debt arithmetic, quantization, and settlement errors map to one fixed sanitized application calculation reason. They must not panic, substitute zero, or return partial calculations.
...
- Rate identity is `(source currency, target currency, original requested date, effective fetch date)`. Stale fallback must match that complete context; fallback across contexts is forbidden.
- Stable historical and refreshable current/future cache classes are each capped at 4,096 entries and use deterministic LRU eviction. Past historical entries may live for the process lifetime and remain stale-eligible without an age limit. Current-date and future-date contexts refresh on UTC day rollover and remain stale-eligible only through seven UTC calendar days after their effective fetch date. Eviction may cause refetching but must not alter correctness or cross-context boundaries.
...
- Render semantic HTML with Askama and vanilla CSS. Pinned self-hosted HTMX plus its pinned official `response-targets` extension are the only permitted client-side libraries and may progressively enhance valid native links/forms. The extension declaratively routes expected `4xx`/`5xx` fragments to stable announced status targets; custom HTMX extensions are forbidden. Every core interaction retains a full-page path when HTMX is unavailable. Custom application JavaScript, inline scripts, and inline script attributes are forbidden.
- One shared strict form/CSRF/submission-token extractor rejects malformed, missing, duplicate, and unknown fields before route-specific parsing or use-case dispatch. CSRF rejection likewise occurs before dispatch. Every rendered unsafe form receives a bounded, expiring, session-bound single-use submission token distinct from CSRF. Validation before dispatch preserves it; immediately before dispatch the server atomically reserves it. Missing, unknown, expired, reserved, or consumed tokens return `409 Conflict` without invoking a use case. The token prevents duplicate dispatch and does not replay a prior response as an idempotency key.
...
- Successful login rotates both session ID and CSRF token and saves the new state before redirect. Logout flushes the session. Expired records are physically removed every five minutes through an indexed expiry structure rather than full-map request scans; cleanup failure fails readiness and initiates bounded shutdown.
...
- Graceful shutdown stops admission, drains admitted work for at most ten seconds, attempts a bounded WAL checkpoint, closes the pool, and leaves WAL sidecars intact if checkpointing fails so SQLite can recover on restart.
