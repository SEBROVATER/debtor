...
`/healthz` is process liveness. `/readyz` checks SQLite and mandatory in-process supervisor health only, never Frankfurter availability or ledger contents. Startup validates configuration and the complete bounded Argon2id v19 policy before connecting or migrating SQLite. Shutdown stops admission, drains for at most ten seconds, attempts a bounded WAL checkpoint, closes the pool, and preserves WAL sidecars if checkpointing fails so SQLite can recover them on the next start. Logs are structured and secret-safe.
