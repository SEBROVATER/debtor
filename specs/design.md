...
- The group page MUST offer atomic create-and-join for new participants and one expense form with independent single/multiple-payer and equal/exact-share choices. These input modes are not persisted; edit screens infer the closest mode from stored allocations. Ordinary spending history MUST use fixed 25-item keyset pagination ordered by `(spent_date DESC, id DESC)`; detail/edit/delete MUST load one complete aggregate directly rather than materializing all group history.
